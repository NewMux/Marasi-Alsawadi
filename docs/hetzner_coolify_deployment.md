...
The migration extends users with local credentials and the `super_admin` role, creates hashed sessions, creates fee definitions and rate assignments, adds ticket subtotal/fee totals, creates immutable ticket lines, and backfills one base line for existing tickets without inventing historical fees. Apply `0007_fix_reservation_omr_precision.sql` after 0006 to preserve two-decimal OMR reservation rates and totals. Apply `0008_add_prd_ticketing_model.sql` after 0007 — it adds the Waterpark/Companion ticket-type model, the continuous non-date ticket-number sequence, group-discount tiers, and seeds the PRD default prices (3 OMR Waterpark, 2 OMR Companion) and discount tiers (25–29→15%, 50–99→25%, 100+→50%); a Super Admin can edit all of these afterward from Commercial Settings. Apply `0009_add_expense_receipt_attachment_and_categories.sql` after 0008 — it adds `receiptNumber`/`attachmentPath`/`attachmentOriginalName` to expense records and seeds the client’s actual accounting-sheet expense categories (Salaries, Utilities, Maintenance, COGS, Advertising & Marketing, Office Supplies, Fixture & Furniture, Tax & VAT, Petty Cash, Other Expenses); a Super Admin can still add/remove categories afterward. Apply `0010_fix_guests_id_reference_column.sql` after 0009 — it renames the `guests` table's legacy `idNumber` column to `idReference` to match `drizzle/schema.ts`; without it, every guest/customer creation (walk-in ticket purchases, the standalone "New customer" form) fails outright, since drizzle's MySQL insert always lists every schema-declared column. Apply `0011_add_expense_adjustments.sql` after 0010 — it creates the `expense_adjustments` table backing the Finance page's category +/- adjustments and category-to-category transfers. Apply `0012`–`0015` after 0011 — they widen OMR money columns to 3-decimal (baisa) precision and add the revenue-category/revenue-record/revenue-adjustment tables mirroring the expense side. Apply `0016_add_ticket_purchase_refunds.sql` after 0015 — it adds `status`/`refundedAt`/`refundedBy` to `ticket_purchases`, backing the Ticket Desk's "Return" action (a customer bringing a ticket back is marked refunded and its linked revenue entry is removed, rather than deleted from the ledger). Apply `0017_add_assets.sql` after 0016 — it creates the `asset_categories`/`asset_records`/`asset_adjustments` tables (a third ledger alongside Expenses and Revenue for capital purchases, deliberately never posted to `finance_entries` so it never affects the Net Result), seeds Fixed Assets/Fixtures & Furniture/Waterpark Infrastructure categories, and retires the old "Fixture & Furniture" expense category (existing expense records under it are left untouched). Apply `0018_add_petty_cash_funds.sql` after 0017 — it adds a `petty_cash` value to the `users.role` enum and creates the `petty_cash_funds`/`petty_cash_spends` tables backing the Petty Cash page: a manager hands a fixed float to a dedicated `petty_cash`-role account (created from that page, not through the Users tab), which can only ever see its own fund and log spending — never change the fixed amount — and each spend posts a real expense record under the existing "Petty Cash" category. Apply `0019_widen_record_category_name_columns.sql` after 0018 — it widens `categoryName` on `expense_records`/`revenue_records`/`asset_records` from 96 to 160 characters to match the category tables' actual limit; without it, any record entered against the "Petty Cash" category (124 characters) fails outright. Do not apply any of these migrations to an existing project’s database.
...
**PRD Round 8, Section 1:** applying a new migration to the live database used to be this manual, easy-to-forget "Deployment operator" checklist step — the direct, repeated cause of "insert fails" bug reports on `guests`, `revenue_categories`, `expense_records`/`asset_records`/`revenue_records`, and `ticket_types`, every time a round shipped a schema change nobody had separately applied to production yet. `server/_core/index.ts` now calls the same migration runner (`applyLegacyMigrations`) automatically before the server starts accepting requests, so **every deploy is self-migrating** — a Coolify redeploy applies whatever new `drizzle/migrations/*.sql` files shipped in that release with no manual step, and it's a fast no-op on a database that's already current. The manual "Apply migration NNNN" checklist rows below are kept only as a historical record of what each migration did; they are no longer a required deployment action.
