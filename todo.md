...
- [x] Verify the active Vercel production alias resolves to commit `5b50d44` through the correct team account or deployment metadata, then record the deployment URL and commit evidence in the handover notes. (Superseded by the owner’s decision to use Hetzner and Coolify for production hosting.)
- [x] If the active Vercel alias is not on `5b50d44`, redeploy GitHub `main` and verify the alias after the deployment completes. (Superseded by the owner’s decision to use Hetzner and Coolify for production hosting.)
...
- [x] Add a persistent admin-managed Oman OMR rate catalog for resort and Aqua Park services, with active/inactive controls and an authorised ticket-desk manual-price override. (Reviewed additive migration applied and verified; staff must select active rates while managers/administrators retain controlled overrides.)
- [x] Add professional ticket-desk rate selection and rate-catalog administration, then validate OMR totals and phone presentation. (Rate history is linked to tickets; used rates are retired rather than deleted; desktop and 375 px demo checks show the complete workflow.)
- [ ] Validate the authenticated `/tickets` rate selector with an active rate, staff-required rate selection, manager/admin manual override, and OMR total recalculation.
- [ ] Validate the authenticated `/finance` rate-catalog controls at desktop and phone widths, including add, edit, retire/remove, and historical ticket-rate visibility.
- [ ] Save the product-focused Oman rate release; defer Hetzner, Coolify, OAuth, and production deployment activities until the website itself is complete.
- [x] Save and push the complete current product state to the authorized NewMux GitHub `main` branch without fabricating unresolved Oman rate values. (Checkpoint `8ace7fdb` pushed and verified on GitHub `main` at `8ace7fdb7c1d6ec07fbd9fc9727fe1971cdcf71e`.)
