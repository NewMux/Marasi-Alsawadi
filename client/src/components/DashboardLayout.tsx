...
function ConnectionStatus({ t }: { t: (key: any) => string }) {
  const online = useConnectionStatus();
  return <span className={cx("inline-flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-[11px] font-semibold", online ? "bg-success-bg text-success" : "bg-danger-bg text-danger")}><span className={cx("h-1.5 w-1.5 rounded-full", online ? "bg-success" : "bg-danger")}/>{online ? t("layout.online") : t("layout.offline")}</span>;
}
