...
export function PrimaryButton({ children, pending, className = "", ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { pending?: boolean }) { return <Button {...props} disabled={pending || props.disabled} className={cx("h-11 rounded-full bg-accent px-5 font-medium text-white shadow-[0_7px_18px_rgba(0,113,227,.2)] hover:bg-accent-hover", className)}>{pending ? <><Loader2 size={15} className="mr-2 animate-spin"/>Saving…</> : children}</Button>; }
