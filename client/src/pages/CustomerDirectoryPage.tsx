...
export default function CustomerDirectoryPage() {
  ...
  const hasSearched = Boolean(query.trim() || countryFilter.trim());
  const { data: records = [], isLoading: customersLoading, isError: customersError } = trpc.platform.customers.search.useQuery(searchInput, { enabled: hasSearched });
  const { data: purchaseRows = [], isLoading: purchasesLoading } = trpc.platform.tickets.purchaseList.useQuery({ query: query.trim() || undefined }, { enabled: hasSearched });
  const isLoading = hasSearched && (customersLoading || purchasesLoading);
  ...
  return <>
    ...
    <Surface className="mt-6"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div className="flex flex-1 flex-col gap-3 sm:flex-row"><div className="flex-1"><SearchField value={query} onChange={setQuery} placeholder={t("customers.searchPlaceholderReal")}/></div><SelectField value={countryFilter} onChange={(event) => setCountryFilter(event.target.value)} className="sm:w-48"><option value="">{t("customers.allCountries")}</option>{COUNTRIES.map((country) => <option key={country} value={country}>{country}</option>)}</SelectField></div><div className="text-xs text-subtle">{!hasSearched ? t("customers.startTypingHint") : isLoading ? t("customers.searching") : `${customers.length} ${customers.length === 1 ? t("customers.profileUnit") : t("customers.profileUnitPlural")}`}</div></div></Surface>
