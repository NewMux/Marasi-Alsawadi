...
export default function CustomerDirectoryPage() {
  ...
  return <>
    ...
    <Surface className="mt-6"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div className="flex flex-1 flex-col gap-3 sm:flex-row"><div className="flex-1"><SearchField value={query} onChange={setQuery} placeholder="Search customer name, phone, email, or ticket number"/></div><SelectField value={countryFilter} onChange={(event) => setCountryFilter(event.target.value)} className="sm:w-48"><option value="">All countries</option>{COUNTRIES.map((country) => <option key={country} value={country}>{country}</option>)}</SelectField></div><div className="text-xs text-subtle">{isLoading ? "Searching…" : `${customers.length} profile${customers.length === 1 ? "" : "s"}`}</div></div></Surface>
    ...
    <Dialog open={creating} onOpenChange={(open) => { setCreating(open); if (!open) setNewCustomer(blankNewCustomer); }}>
      <DialogContent>
        ...
        <div className="grid gap-4 py-2">
          ...
          <Field label="Country"><SelectField value={newCustomer.nationality} onChange={(event) => setNewCustomer({ ...newCustomer, nationality: event.target.value })}>{COUNTRIES.map((country) => <option key={country} value={country}>{country}</option>)}</SelectField></Field>
        </div>
