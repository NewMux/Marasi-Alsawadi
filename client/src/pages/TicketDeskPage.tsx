...
// PRD Round 10: VAT is now a per-ticket-type rate rather than one hardcoded
// 5%, but neither ticket_purchases nor ticket_purchase_lines snapshot the
// rate itself (only the resulting vatAmount) — the rate is instead derived
// here from the stored amounts so both a freshly created purchase and an
// older reprinted one show a correct dynamic percentage with no migration.
function derivedVatPercentage(vatAmount: unknown, basePrice: unknown, discountAmount: unknown) {
  const discounted = Number(basePrice || 0) - Number(discountAmount || 0);
  return discounted > 0 ? ((Number(vatAmount || 0) / discounted) * 100).toFixed(2) : "0.00";
}
...
export default function TicketDeskPage() {
  ...
  // PRD Round 9, Sections 4 and 6: the preview used to require every entered
  // line to be priceable before it would run at all, so one unpriced category
  // — or simply not having picked a ticket type yet — left it sitting at
  // 0.000 with nothing on screen explaining why. It now prices whatever is
  // complete, as it is entered, and names what is holding the rest up.
  const blockingReasons = useMemo(() => {
    if (!ticketTypesInGroup.length) return [];
    ...
    if (mode === "individual") {
      ...
      groupLines.forEach((line, index) => {
        const quantity = Math.floor(line.quantity || 0);
        if (!line.categoryId || quantity < 1) { if (line.categoryId || quantity) reasons.push(t("tickets.incompleteGroupLine", { index: index + 1 })); return; }
        if (!priceIdFor(Number(line.categoryId))) reasons.push(t("tickets.unpricedCategory", { name: visitorCategories.find((entry) => String(entry.id) === line.categoryId)?.name || line.categoryId }));
      });
    }
    return reasons;
  ...
  // apart. The error is now shown verbatim so staff (and we) can see why.
  const { data: pricing, error: previewError } = trpc.platform.tickets.purchasePreview.useQuery({ lines: previewLines, partnerEntityId: partnerEntityId ? Number(partnerEntityId) : undefined }, { enabled: previewLines.length > 0 && previewLines.length <= maxTicketsPerPurchase, retry: false });
  const partnerDiscountMissing = Boolean(pricing?.partnerEntity) && effectiveTicketTypeId
    ? !Number(pricing?.partnerDiscountByTicketType?.[effectiveTicketTypeId] ?? 0)
    : false;
        ...
        {mode === "individual" ? <div className="grid gap-3">{visibleVisitorCategories.map((category) => {
          ...
          return <div key={category.id} className={cx("flex items-center justify-between gap-4 rounded-2xl border bg-well p-4", rowMissingPrice ? "border-danger" : "border-divider")}>
            <div className="min-w-0">
              ...
              {/* PRD Round 9, Section 4: with no ticket type chosen there is no
                  price to look up — say that, rather than reporting every
                  category as unconfigured. */}
              <div className="mt-1 text-[11px] leading-4 text-muted">{!effectiveTicketTypeId ? t("tickets.chooseTicketTypeFirst") : !price ? t("tickets.noPriceConfigured") : Number(price.unitPrice) === 0 ? t("tickets.freeHint") : money(price.unitPrice)}{maxPerBooking ? ` · ${t("tickets.maxPerBookingHint", { max: maxPerBooking })}` : ""}</div>
            </div>
