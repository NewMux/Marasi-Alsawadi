...
export default function FinanceControlPage() {
  ...
  // PRD Round 15, Section 7.1: the Record Transaction tab's own Revenue
  // category options — the live Ticket Type + Facility Type list, not the
  // general revenue_categories library above (still used, unchanged, by the
  // Adjust/Transfer tabs and category balances).
  const { data: revenueCategoryOptions = [], isLoading: revenueCategoryOptionsLoading } = trpc.platform.finance.revenueCategoryOptionsForRecording.useQuery(undefined, { enabled: canRecord });
    ...
    {(!canManage || screenTab === "record") && <div className="mt-6 grid gap-6 xl:grid-cols-[.8fr_1.2fr]">
      <Surface>
        ...
        <div className="grid gap-4">
          ...
          <Field label={t("common.category")}><SelectField value={transactionForm.categoryId} onChange={(event) => setTransactionForm({ ...transactionForm, categoryId: event.target.value })}><option value="">{t("finance.chooseCategory")}</option>{transactionForm.categoryType === "revenue" ? (revenueCategoryOptions as any[]).map((option) => <option key={option.categoryId} value={option.categoryId}>{option.name}</option>) : (categoriesForType(transactionForm.categoryType) as any[]).map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</SelectField>{(categoriesLoading || revenueCategoryOptionsLoading || assetCategoriesLoading) && <span className="text-[11px] text-subtle">{t("finance.loadingCategories")}</span>}{transactionForm.categoryType === "revenue" && <span className="text-[11px] leading-4 text-subtle">{t("finance.revenueCategoriesFollowTypes")}</span>}{transactionForm.categoryType !== "revenue" && isSuperAdmin && <span className="text-[11px] leading-4 text-subtle">{t("finance.categoriesInSettings")}</span>}</Field>
